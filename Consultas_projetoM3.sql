...
SELECT nome.release_date, sum(nome.contador) 
FROM (SELECT release_date, count(release_date) as contador 
      FROM steam
      WHERE release_date 
      LIKE "1998%") 
...
SELECT genero.genres, sum(genero.contador) 
FROM (SELECT genres, count(genres) as contador 
      FROM steam where genres like '%ACTION%') 
as genero group by genero.contador;
